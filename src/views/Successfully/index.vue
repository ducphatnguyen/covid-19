...
const questionList = computed(() => {
  const country = countries.value.find((country: Country) => country.code === countryCode);
  return country ? country.questionList : [];
});

const type = computed(() => {
  if (questionList.value.length) {
    return compareResults(checklistAnswers, questionList.value) ? "fail" : "success";
  }
});
...
<template>
  <a-flex class="successfully" vertical>
    ...
    >
      ...
      <a-button style="min-width: 120px" size="large" @click="goToReviewPage" v-if="!payloadStore.$state.isReviewed">
